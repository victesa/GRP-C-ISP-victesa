 ...
 *
 * @module
 */ import { alphabet, calcSizeHex, decode, encode, rAlphabet } from "./_common16.js";
export { calcSizeHex };
 ...
 */ export function encodeHex(input) {
  ...
  const [output, i] = detach(input, calcSizeHex(input.length));
 ...
 * );
 * ```
 */ export function encodeIntoHex(input, output) {
  if (typeof input === "string") {
    input = new TextEncoder().encode(input);
  } else if (input instanceof ArrayBuffer) {
    input = new Uint8Array(input);
  }
  const min = calcSizeHex(input.length);
...
//# sourceMappingURL=unstable_hex.js.map