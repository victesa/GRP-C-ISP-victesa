 ...
 *
 * @module
 */ import { alphabet, calcSizeBase32, decode, encode, padding, rAlphabet } from "./_common32.js";
export { calcSizeBase32 };
 ...
 * );
 * ```
 */ export function encodeBase32(input, options = {}) {
  ...
  const [output, i] = detach(input, calcSizeBase32(input.length));
 ...
 * );
 * ```
 */ export function encodeIntoBase32(input, output, options = {}) {
  options.alphabet ??= "base32";
  if (typeof input === "string") {
    input = new TextEncoder().encode(input);
  } else if (input instanceof ArrayBuffer) {
  ...
  const min = calcSizeBase32(input.length);
 ...
 * );
 * ```
 */ export function decodeBase32(input, options = {}) {
  options.alphabet ??= "base32";
  if (typeof input === "string") {
    input = new TextEncoder().encode(input);
  }
  return input.subarray(0, decode(input, 0, 0, rAlphabet[options.alphabet], padding));
}
//# sourceMappingURL=unstable_base32.js.map